...
print("lst4 :", lst4)
print("======= 리스트 컴프리헨션으로 0~10까지의 숫자의 짝수의 제곱을 원소로 가지는 리스트를 작성하라. ======")
lst4 = [i**2 for i in range(11) if i%2==0]
print("lst4 :", lst4)
...
print("======= 깊은 복사와 얕은 복사 ======")
l1 = [1,2,3,4]
l3 = l1.copy() #<----- 독립적 메모리 할당. 딥 카피(깊은 복사)
l3 = l1[:] #<----- 독립적 메모리 할당. 딥 카피(깊은 복사)
l3 = list(l1) #<----- 독립적 메모리 할당. 딥 카피(깊은 복사)
print("pop()하기 전의 l1 :", l1)
l2=l1 #쉘로우 카피(얕은 복사). 둘 모두 같은 메모리를 공유함.
l1.pop() # l1을 pop()해버리니 같은 곳을 바라보는 l2도 마지막 원소가 함게 날아간 걸 확인 가능.
print("pop()한 이후의 l1 :", l1)
print("pop()한 이후의 l2 :", l2)
print("pop()한 이후의 l3 :", l3)
print("======= 메모리의 위치를 확인하는 방법 ======")
print("l1과 l2는 같은 메모리를 공유한다 :", l1 is l2) # <------ 이건 l1와 l2가 같은 메모리를 바라보고 있으면 True, 아니면 False가 나온다.
print("l1과 l3는 같은 메모리를 공유한다 :", l1 is l3)
